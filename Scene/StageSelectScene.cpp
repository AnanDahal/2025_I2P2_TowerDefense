...
void StageSelectScene::Initialize() {
    ...
    // int offsetY = 100; // for the bgm and the sfx to be lower
    // Slider *sliderBGM, *sliderSFX;
    // sliderBGM = new Slider(40 + halfW - 95, halfH - 50 - 2 + offsetY, 190, 4);
    // sliderBGM->SetOnValueChangedCallback(std::bind(&StageSelectScene::BGMSlideOnValueChanged, this, std::placeholders::_1));
    // AddNewControlObject(sliderBGM);
    // AddNewObject(new Engine::Label("BGM: ", "pirulen.ttf", 28, 40 + halfW - 60 - 95, halfH - 50 + offsetY, 255, 255, 255, 255, 0.5, 0.5));
    // sliderSFX = new Slider(40 + halfW - 95, halfH + 50 - 2 + offsetY, 190, 4);
    // sliderSFX->SetOnValueChangedCallback(std::bind(&StageSelectScene::SFXSlideOnValueChanged, this, std::placeholders::_1));
    // AddNewControlObject(sliderSFX);
    // AddNewObject(new Engine::Label("SFX: ", "pirulen.ttf", 28, 40 + halfW - 60 - 95, halfH + 50 + offsetY, 255, 255, 255, 255, 0.5, 0.5));
